...
export const usePanelOvertimeBulkStore = defineStore(
  'panelOvertimeBulkStore',
  () => {
    ...
    const onToggleAllChecked = () => {
      if (isHasItemsChecked.value) {
        ...
        itemsChecked.value = panelOvertimeListStore.itemsOvertime?.map(
          (obj: any) => {
            return obj._id;
          },
        );
    ...
    const onSubmitForm = async () => {
      ...
      inputForm.from_date = panelOvertimeListStore.inputFilter.start_date;
      inputForm.to_date = panelOvertimeListStore.inputFilter.end_date;
      ...
      dialogStore.open({
        ...
        onOk: async () => {
          ...
          const res: any = await axios.patch('v1/overtimes/bulk', inputForm);
